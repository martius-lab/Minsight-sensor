...
def error_calculation(index, i=None, force_map=False):
    ...
    for i in tqdm(index):
        ...
        if force_map:
            map_label = torch.load(
                os.path.join(data_path + params.map_dir, "Map2_" + str(i) + ".txt")
            )
        ...
        if force_map:
            loc_acc_total, force_acc_total = calc_errors_torch(
                postprocessor.convert_map_to_vec(output),
                postprocessor.convert_map_to_vec(map_label),
            )
            loc_acc_raw, force_acc_raw = calc_errors_torch_raw(
                postprocessor.convert_map_to_vec(output),
                postprocessor.convert_map_to_vec(map_label),
            )
            ...
            loc_acc_total, force_acc_total = calc_errors_torch(
                postprocessor.undo_rescale(output), label
            )
            loc_acc_raw, force_acc_raw = calc_errors_torch_raw(
                postprocessor.undo_rescale(output), label
            )
...
def plot_total_errors_cont(force_error, limit, nodes, title, path, cmap="YlOrBr"):
    ...
    fig, (ax1, ax2) = plt.subplots(
        1, 2, gridspec_kw={"width_ratios": [1.05, 2]}, figsize=(8, 3.7)
    )
    x = nodes[:, 1]
    y = nodes[:, 2]
    z = force_error
    ...
    ax1.scatter(
        skeleton_surface[:, 1], skeleton_surface[:, 2], s=0.3, c="gray", alpha=1.0
    )
    plt.xlim(-20, 20)
    plt.ylim(-20, 20)
    ax1.axis("equal")
    ...
    ax2.scatter(
        skeleton_surface[:, 2], skeleton_surface[:, 3], s=0.3, c="gray", alpha=1.0
    )
    plt.xlim(-20, 20)
    plt.ylim(-5, 35)
    plt.axis("equal")
...
def plot_3d_errors_force(force_error, force_mag, limit, title, path, cmap="BuPu"):
    ...
    error_04 = np.array(
        [error for error, mag in zip(force_error, force_mag) if mag <= 0.4]
    )
    error_08 = np.array(
        [
            error
            for error, mag in zip(force_error, force_mag)
            if mag <= 0.8 and mag > 0.4
        ]
    )
    error_12 = np.array(
        [
    ...
    error_abs = [
        np.linalg.norm(error_04, axis=1),
        np.linalg.norm(error_08, axis=1),
        np.linalg.norm(error_12, axis=1),
    ]
    ...
    vp1 = ax.violinplot(
        error_x,
        points=100,
        positions=np.arange(0.2, len(error_x) + 0.2),
        showmeans=False,
        showmedians=False,
        showextrema=False,
    )
    ...
    vp2 = ax.violinplot(
        error_y,
        points=100,
        positions=np.arange(0.1, len(error_y) + 0.1),
        showmeans=False,
        showmedians=False,
        showextrema=False,
    )
    ...
    vp3 = ax.violinplot(
        error_z,
        points=400,
        positions=np.arange(0.0, len(error_z)),
        showmeans=False,
        showmedians=False,
        showextrema=False,
    )
    ...
    vp4 = ax.violinplot(
        error_abs,
        points=400,
        positions=np.arange(-0.1, len(error_abs) - 0.1),
        showmeans=False,
        showmedians=False,
        showextrema=False,
    )
    ...
    plt.legend(
        [vp4["bodies"][0], vp3["bodies"][0], vp2["bodies"][0], vp1["bodies"][0]],
        ["Total\nforce", "Shear\nforce 1", "Shear\nforce 2", "Normal\nforce"],
        ncol=4,
        loc="lower left",
        prop={"size": 8},
...
def plot_3d_errors_position(force_error, force_mag, limit, title, path, cmap="BuPu"):
    ...
    error_1up = np.array(
        [error for error, mag in zip(force_error, force_mag) if mag > 1.0]
    )
    ...
    vp4 = ax.violinplot(
        error_abs,
        points=400,
        positions=np.arange(0, len(error_abs)),
        showmeans=False,
        showmedians=False,
        showextrema=False,
    )
...
if __name__ == "__main__":
    ...
    if args.calc == True:
        ...
        (
            ...
            loc_errors_3d,
            force_errors_3d,
        ) = error_calculation(test_index, force_map=params.force_map)

        np.save(
            os.path.join(result_path, "loc_accuracies.npy"),
            [loc_errors_total, loc_errors_3d],
        )
        np.save(
            os.path.join(result_path, "force_accuracies.npy"),
            [force_errors_total, force_errors_3d],
        )
        np.save(os.path.join(result_path, "force_magnitudes.npy"), force_mag)
    else:
        loc_errors = np.load(
            os.path.join(result_path, "loc_accuracies.npy"), allow_pickle=True
        )
        force_errors = np.load(
            os.path.join(result_path, "force_accuracies.npy"), allow_pickle=True
        )
        force_mag = np.load(
            os.path.join(result_path, "force_magnitudes.npy"), allow_pickle=True
        )

    total_loc_errors = process_data(loc_errors[0])
    total_force_erros = process_data(force_errors[0])

    plot_total_errors_cont(
        np.array(force_errors[0], dtype=object),
        0.5,
        nodes[nodes[:, 0].argsort()],
        "Force Error [N]",
        result_path,
        cmap="YlOrBr",
    )
    plot_total_errors_cont(
        np.array(loc_errors[0], dtype=object),
        5.0,
        nodes[nodes[:, 0].argsort()],
        "Position Error [mm]",
        result_path,
    ...
    plot_3d_errors_position(
        np.array([np.array(err) for err in np.array(loc_errors[1:])]),
        force_mag,
        5.0,
        "Position Error Distribution multi [mm]",
        result_path,
        cmap="BuPu",
    )
    plot_3d_errors_force(
        np.array([np.array(err) for err in np.array(force_errors[1:])]),
        force_mag,
        0.5,
        "Force Error Distribution multi [N]",
        result_path,
        cmap="YlOrBr",
    )
