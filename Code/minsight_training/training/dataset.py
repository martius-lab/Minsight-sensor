...
class ForceDataset(Dataset):
    ...
    def __init__(self, root_dir, indices, params):
        ...
        self.X4 = TF.to_tensor(
            resize(
                cv2.imread(
                    (os.path.join(self.image_dir, "Resized_Position_reference.png"))
                ),
                self.scale,
            )
        )
    ...
    def __getitem__(self, idx):
        ...
        img_name = os.path.join(
            self.image_dir, "Resized" + str(self.indices[idx]) + ".png"
        )
        ...
        if self.force_map:
            label = torch.load(
                os.path.join(self.map_dir, "Map2_" + str(self.indices[idx]) + ".txt")
            )
            if self.rescale:
                label = label * 4096.0
        else:
            force = np.load(
                self.image_dir + "Reformulated_force" + str(self.indices[idx]) + ".npy"
            )
            if self.rescale:
...
def ForceDataloaders(params, num_workers=4):
    ...
    print(
        "Train Set: : %i, Test Set: %i, Validation Set: %i"
...
class Postprocessor:
    ...
    def __init__(self, use_gpu, params):
        ...
        self.image_dir = root_dir + "PostData/"
        X3 = resize(
            cv2.imread(os.path.join(self.image_dir, "Resized_NoContact.png")),
            self.img_scale,
        )
        ...
        self.indentation_info = torch.from_numpy(
            np.load(os.path.join(root_dir, "indentation_info.npy"))
        )
        ...
        if params.force_map == True:
            ...
            self.col_ind = torch.from_numpy(
                np.load(
                    os.path.join(root_dir + params.map_dir, "column_idx_mapping.npy")
                )
            )
        else:
            ...
            self.scale = torch.from_numpy(
                1.0 / 1024.0 * (self.max_ - self.min_)
            ).float()
    ...
    def convert_map_to_vec(self, pred, num_highest_loc=20):
        ...
        if self.flat_output == False:
            for i in range(l):
                ...
                force = self.indentation_info[
                    torch.argsort(
                        torch.linalg.norm(map_[:, self.col_ind], axis=0),
                        descending=True,
                    )[:num_highest_loc],
                    1:4,
                ]
            ...
            for i in range(l):
                ...
                force = self.indentation_info[
                    torch.argsort(
                        torch.linalg.norm(map_[:, :], axis=0), descending=True
                    )[:num_highest_loc],
                    1:4,
                ]
                px[i] = torch.mean(force[:, 0])
